<template>
    <view>
		...
		<uni-list-item v-for="item in dataList">
			...
			<view></view>
			<text slot="body" class="slot-box slot-text">水温:{{ item.tWaterTemperature }};水位:{{ item.tWaterLevel }};\n浊度:{{ item.tTurbidity }}</text>
	...
	export default {
	    ...
	    data() {
	        return {
	            ...
	            rangetime: ['2021/02/07 14:00:00','2021/02/09 13:59:00'],
	            type: 'rangetime',
	            value: '',
				dataList:undefined,
				total: 0,
	    ...
	    methods: {			
			...
			getData(page = 0){
				...
				uni.request({
					url: 'getHistoryData?startDate='+this.rangetime[0]+"&stopDate="+this.rangetime[1]+"&page="+ page+"&size="+this.pageSize,
					// data: data,
					success: data => {
						console.log(data);
						if(data.data.code == 1){
