			  ...
			  <el-carousel style="width: 90%;margin-left: 5%;margin-top: 30px;" :interval="4000" type="card" height="200px">
			    <el-carousel-item v-for="item in 6" style="width: 400px;" :key="item">
			         <el-image
			            ...
			            :src="goodsCreatList[item-1].photoUrl"
			            fit="scale-down"  @click="imgClick(goodsCreatList[item-1].id)">
						</el-image>
					<div style="text-align: center;">{{goodsCreatList[item-1].goodsName}}</div>
					<div style="text-align: center;font-size: 10px;">上架时间：{{formatTimer(goodsCreatList[item-1].createTime)}}</div>
			    </el-carousel-item>
			  </el-carousel>
		</div>
			  ...
			  <el-carousel style="width: 90%;margin-left: 5%;margin-top: 30px;" :interval="4000" type="card" height="200px">
			    <el-carousel-item v-for="item in 6" style="width: 400px;" :key="item">
			         <el-image
			            ...
			            :src="goodsList[item-1].photoUrl"
			            fit="scale-down"  @click="imgClick(goodsList[item-1].id)">
						</el-image>
					<div style="text-align: center;">{{goodsList[item-1].goodsName}}</div>
					<div style="text-align: center;font-size: 10px;">借阅热度：{{goodsList[item-1].selectNumber}}</div>
			    </el-carousel-item>
			  </el-carousel>
		</div>
...
<script>
  import miuiver from "@/api/miuiver";
	import goodsAxios from "@/api/goods";
  ...
  export default {
    ...
    methods: {
		getCreatList(page = 1) {
			this.page = page
			goodsAxios.getGoodsInfoCreatListAxios(this.page, this.size).then(resp => {
				console.log(resp)
				// this.state = 4
				if (resp.code === 20000) {
					// this.$message.success(resp.message)
					this.goodsCreatList = resp.data.list
		...
		getgoodsList(page = 1) {
			this.page = page
			goodsAxios.getGoodsInfoListAxios(this.page, this.size).then(resp => {
				console.log(resp)
				// this.state = 4
				if (resp.code === 20000) {
					// this.$message.success(resp.message)
	...
	// this.$router.push({name:'/goods/goodsInfo/'+item})
	
		this.$router.push('/goods/goodsInfo/'+item)
	},
